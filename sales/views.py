...
def add(request):
    if request.method == "POST":
        if request.POST.get('act') == "create_order":
            cursor = connection.cursor()
            cursor.execute(
                "INSERT INTO `order` (order_customer_name, order_customer_mobile, order_date) VALUES (%s, %s, %s)",
                (
        ...
        elif request.POST.get('act') == "save_order":
            ...
            cursor.execute(
                "INSERT INTO order_item (oi_order_id, oi_product_id, oi_cost) VALUES (%s, %s, %s)",
                (
...
def getProductDetails(orderID):
    if orderID:
        ...
        if dataList:
            ...
            return None  # or any other appropriate action when no data is found
    return None  # or any other appropriate action when orderID is not provided
...
def update(request, orderId):
    context = {
        "fn": "update",
        "orderdetails": getData(orderId),
    ...
    if request.method == "POST":
        ...
        cursor.execute("""
            UPDATE sales_sales
            SET order_car_name = %s,
                order_customer_name = %s,
                order_date = %s,
                order_cars = %s
            WHERE order_id = %s
        """, (
            request.POST['order_car_name'],
            request.POST['order_customer_name'],
            request.POST['order_date'],
            request.POST['order_cars'],
            orderId
        ))
        context["orderdetails"] = getData(orderId)
        messages.add_message(request, messages.INFO, "Order updated successfully!")
